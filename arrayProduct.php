<?php


$productos = array(
    'Bombones' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Chocolate macizo con leche",
            'descripcion' => "",
            // Chocolate macizo con leche
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/macizo_leche.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"

        ),
        1 => array(
            'id' => 2,
            'nombre' => "Relleno De Café",
            'descripcion' => "", 
            // Relleno De Café
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/relleno-cafe.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Relleno de cajeta",
            'descripcion' => "",
            // Relleno de cajeta
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/cajeta.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        3 => array(
            'id' => 4,
            'nombre' => "Relleno de cereza con licor",
            'descripcion' => "",
            // Relleno de cereza con licor
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/cereza.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        4 => array(
            'id' => 5,
            'nombre' => "Relleno de crema de cacahuate",
            'descripcion' => "",
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/crema_cacahuate.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "0"
        ),
        5 => array(
            'id' => 6,
            'nombre' => "Relleno de crema de cacahuate",
            'descripcion' => "",
            // Relleno de crema de cacahuate
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/crema_cacahuate.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        6 => array(
            'id' => 7,
            'nombre' => "Relleno de  vainilla",
            'descripcion' => "",
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/vainilla.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        7 => array(
            'id' => 8,
            'nombre' => "Relleno de fresa",
            'descripcion' => "",
            // Relleno de fresa
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/fresa.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        8 => array(
            'id' => 9,
            'nombre' => "Relleno de guanábana",
            'descripcion' => "",
            // Relleno de guanábana
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/guanabana.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        9 => array(
            'id' => 10,
            'nombre' => "Relleno De Cereza",
            'descripcion' => "",
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/cajeta.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "0"
        ),
        10 => array(
            'id' => 11,
            'nombre' => "Relleno de mango",
            'descripcion' => "",
            // Relleno de mango
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/mango.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        11 => array(
            'id' => 12,
            'nombre' => "Relleno de maracuyá",
            'descripcion' => "",
            // Relleno de maracuyá
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/maracuya.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        12 => array(
            'id' => 13,
            'nombre' => "Relleno de crema de avellana",
            'descripcion' => "",
            // Relleno de avellana
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/nutella.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        13 => array(
            'id' => 14,
            'nombre' => "Relleno de rompope",
            'descripcion' => "",
            // Relleno de rompope
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/rompope.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        14 => array(
            'id' => 15,
            'nombre' => "Relleno de vino",
            'descripcion' => "",
            // Relleno de vino tinto
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/vino.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        15 => array(
            'id' => 16,
            'nombre' => "Relleno de zarzamora",
            'descripcion' => "",
            // Relleno de zarzamora
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/zarzamora.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        16 => array(
            'id' => 17,
            'nombre' => "Trufas",
            'descripcion' => "Trufas de chocolate semiamargo: sabor natural y moka",
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/trufas.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        17 => array(
            'id' => 18,
            'nombre' => "Pelota de golf chocolate blanco",
            'descripcion' => "Pelota de golf chocolate blanco 16gr",
            'precio' => "$25",
            'imagen' => "/images/productos/golf-bco.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        18 => array(
            'id' => 19,
            'nombre' => "Bolsitas bombones de colores rellenos",
            'descripcion' => "Bolsita Bombones diferentes colores 150 gr",
            'precio' => "$270",
            'imagen' => "/images/productos/bolsita-colores.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "0"
        ),
        19 => array(
            'id' => 20,
            'nombre' => "Fresas con chocolate",
            'descripcion' => "Fresas con chocolate (blanco, de leche o semiamargo)",
            'precio' => "$15-18 pza",
            'imagen' => "/images/productos/fresas.jpg",
            'imagen2' => "/images/productos/fresas-2.jpg",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        20 => array(
            'id' => 21,
            'nombre' => "Bombón relleno de Baileys",
            'descripcion' => "",
            // Bombón relleno de Baileys
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/relleno-baileys.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        21 => array(
            'id' => 22,
            'nombre' => "Bombón relleno de almendra caramelizada",
            'descripcion' => "",
            // Bombón relleno de almendra caramelizada
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/almendra-caramelizada.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        22 => array(
            'id' => 23,
            'nombre' => "Chocolate semiamargo con nuez al 70%",
            'descripcion' => "",
            // Macizo chocolate semiamargo con nuez
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/macizo-nuez.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        23 => array(
            'id' => 24,
            'nombre' => "Chocolate pranilé vegano",
            'descripcion' => "Relleno con pranilé de almendra vegano",
            'precio' => "$23",
            'imagen' => "/images/productos/pranile-vegano.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        24 => array(
            'id' => 25,
            'nombre' => "Bombón relleno de frutos rojos",
            'descripcion' => "",
            // Bombón relleno de frutos rojos
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/frutos-rojos.jpg",
            'imagen2' => "/images/productos/frutos-rojos-2.jpg",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        25 => array(
            'id' => 26,
            'nombre' => "Pelota de golf chocolate con leche",
            'descripcion' => "Pelota de golf chocolate con leche 16gr",
            'precio' => "$25",
            'imagen' => "/images/productos/pelota-leche.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        26 => array(
            'id' => 27,
            'nombre' => "Bombón relleno con CBD",
            'descripcion' => "Chocolate macizo semiamargo con CBD",
            'precio' => "$40",
            'imagen' => "/images/productos/cbd.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        27 => array(
            'id' => 28,
            'nombre' => "Bombas de chocolate",
            'descripcion' => "Bombas de Chocolate 55 gr",
            'precio' => "$100",
            'imagen' => "/images/productos/bomba.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "0"
        ),
        28 => array(
            'id' => 29,
            'nombre' => "Pelota de golf chocolate semiamargo",
            'descripcion' => "Pelota de golf chocolate semiamargo 16gr",
            'precio' => "$25",
            'imagen' => "/images/productos/pelota-oscuro.jpg",
            'imagen2' => "/images/productos/pelota-oscuro2.jpg",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
    ),
    'Bombones Mexicanos' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Bombón relleno individual",
            'descripcion' => "",
            'precio' => "$15 a $19 pesos por pieza",
            'imagen' => "/images/productos/relleno-individual.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "0"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Relleno de damiana",
            'descripcion' => "",
            // Relleno de damiana
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/damiana.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Relleno de mezcal",
            'descripcion' => "",
            // Relleno de mezcal
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/mezcal.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        3 => array(
            'id' => 4,
            'nombre' => "Relleno de mole",
            'descripcion' => "",
            // Relleno de mole
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/mole.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        4 => array(
            'id' => 5,
            'nombre' => "Relleno de naranja caramelizada",
            'descripcion' => "",
            // Relleno de naranja caramelizada
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/naranja.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        5 => array(
            'id' => 6,
            'nombre' => "Relleno de piña coco",
            'descripcion' => "",
            // Relleno de piña coco
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/pina-coco.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        6 => array(
            'id' => 7,
            'nombre' => "Relleno de tamarindo",
            'descripcion' => "",
            // Relleno de tamarindo
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/tamarindo.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        7 => array(
            'id' => 8,
            'nombre' => "Relleno de tamarindo con chile",
            'descripcion' => "",
            // Relleno de tamarindo con chile
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/tamarindo-c-chile.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        8 => array(
            'id' => 9,
            'nombre' => "Relleno de tequila",
            'descripcion' => "",
            // Relleno de tequila
            'precio' => "$15 a $18 pesos por pieza",
            'imagen' => "/images/productos/tequila.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        )
    ),
    'Barras Mexicanas' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Catrines",
            'descripcion' => "Barra Catrina Chocolate Con Leche 60gr",
            'precio' => "$90",
            'imagen' => "/images/productos/catrines.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Chocolate de mesa - pza",
            'descripcion' => "chocolate de mesa con canela, vainilla y almendra 80gr.",
            'precio' => "$85",
            'imagen' => "/images/productos/chocolate-mesa.jpg?=1",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
    ),
    'Barras' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Barra 100% cacao",
            'descripcion' => "Barra de chocolate hecho con cacao criollo almendra blanca y un toque fresco de menta",
            'precio' => "$70 pza",
            'imagen' => "/images/productos/100-cacao.jpg",
            'imagen2' => "",
            'venta' => "Peso",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Barra de chocolate blanco",
            'descripcion' => "Barra de chocolate blanco 45gr",
            'precio' => "$75",
            'imagen' => "/images/productos/barra-blanca.jpg",
            'imagen2' => "/images/productos/blanco-2.jpg",
            'venta' => "Granel",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Barra de chile",
            'descripcion' => "Barra con Chocolate Semiamargo y chile 45 gr",
            'precio' => "$75",
            'imagen' => "/images/productos/barra-chile.jpg",
            'imagen2' => "/images/productos/barra-chile-2.jpg",
            'venta' => "Peso",
            'activo' => "1"
        ),
        3 => array(
            'id' => 4,
            'nombre' => "Barra de chocolate con leche",
            'descripcion' => "Barra Logo con Chocolate de Leche 45 gr",
            'precio' => "$75",
            'imagen' => "/images/productos/barra-logo.jpg",
            'imagen2' => "",
            'venta' => "Peso",
            'activo' => "1"
        ),
        4 => array(
            'id' => 5,
            'nombre' => "Barra de chocolate con menta",
            'descripcion' => "Barra de chocolate hecho con cacao criollo almendra blanca y un toque fresco de menta 100gr",
            'precio' => "$130",
            'imagen' => "/images/productos/cardomomo.jpg",
            'imagen2' => "",
            'venta' => "Peso",
            'activo' => "1"
        ),
        5 => array(
            'id' => 6,
            'nombre' => "Barra de cuadro con logo",
            'descripcion' => "Barra Cuadro Logo con Chocolate de Leche 45 gr",
            'precio' => "$55",
            'imagen' => "/images/productos/cuadro-logo.jpg",
            'imagen2' => "",
            'venta' => "Peso",
            'activo' => "1"
        ),
        6 => array(
            'id' => 7,
            'nombre' => "Barra con cardomomo",
            'descripcion' => "Barra de Chocolate Semiamargo y cardamomo 80 gr",
            'precio' => "$120",
            'imagen' => "/images/productos/menta.jpg",
            'imagen2' => "",
            'venta' => "Peso",
            'activo' => "1"
        ),
        7 => array(
            'id' => 8,
            'nombre' => "Barra diseño papel picado ",
            'descripcion' => "Barra de chocolate de leche y vainilla diseño de papel picado 65gr",
            'precio' => "$95",
            'imagen' => "/images/productos/picado.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        8 => array(
            'id' => 9,
            'nombre' => "Barra vegana",
            'descripcion' => "Leche de almendra y azucar mascabada y es de chocolate obscuro con el 65 porciento cacao. 45g",
            'precio' => "$90",
            'imagen' => "/images/productos/vegana.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        9 => array(
            'id' => 10,
            'nombre' => "Barra completa arco Cabo - pza",
            'descripcion' => "Barra Arco San Lucas con Chocolate Semiamargo mezcla 250 gr",
            'precio' => "$300",
            'imagen' => "/images/productos/barra-arco.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        10 => array(
            'id' => 11,
            'nombre' => "Barra Maya chocolate semiamargo",
            'descripcion' => "Barra de chocolate semiamargo hecho con cacao criollo almendra blanca con diseño maya",
            'precio' => "$80",
            'imagen' => "/images/productos/barra-maya.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        11 => array(
            'id' => 12,
            'nombre' => "Barra con flor chica",
            'descripcion' => "",
            'precio' => "$90 pieza",
            'imagen' => "/images/productos/barra-flores.jpg",
            'imagen2' => "",
            'venta' => "Pieza",
            'activo' => "0"
        ),
        12 => array(
            'id' => 13,
            'nombre' => "Barra mini maltitol 66gr",
            'descripcion' => "Barra al 70% cacao, endulzado con maltitol",
            'precio' => "$85",
            'imagen' => "/images/productos/barra-mini-matitol.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        13 => array(
            'id' => 14,
            'nombre' => "Barra chocolate con sal de mar",
            'descripcion' => "Barra de Chocolate Semiamargo con Sal 70gr",
            'precio' => "$90",
            'imagen' => "/images/productos/semi-amargo-sal.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        14 => array(
            'id' => 15,
            'nombre' => "Barra grande Maltitol",
            'descripcion' => "Barra de chocolate semiamargo diseño greca 79gr",
            'precio' => "$330",
            'imagen' => "/images/productos/barra-maltitol.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        15 => array(
            'id' => 16,
            'nombre' => "Barra con flores grande",
            'descripcion' => "Barra de chocolate semiamargo hecho con cacao diseño de flores 125g",
            'precio' => "$150",
            'imagen' => "/images/productos/barra-flores.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        16 => array(
            'id' => 17,
            'nombre' => "Barra semiamarga con jengibre",
            'descripcion' => "Barra de chocolate semiamargo con jengibre 45g",
            'precio' => "$80",
            'imagen' => "/images/productos/semiamargo-jengibre.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        17 => array(
            'id' => 18,
            'nombre' => "Barra de chocolate semiamargo diseño greca",
            'descripcion' => "Barra de chocolate semiamargo diseño greca 79gr",
            'precio' => "$120",
            'imagen' => "/images/productos/barra-greca.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        
    ),
    'Marqueta' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Chocolate blanco con frutos secos",
            'descripcion' => "Marqueta Chocolate Blanco 250",
            'precio' => "$270",
            'imagen' => "/images/productos/blanco-con-frutos.jpg",
            'imagen2' => "/images/productos/marqueta-2.jpg",
            'venta' => "Por gramaje o kilo",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Chocolate de leche y frutos secos",
            'descripcion' => "Chocolate con leche y frutos secos  250gr",
            'precio' => "$270",
            'imagen' => "/images/productos/marqueta-leche.jpg",
            'imagen2' => "",
            'venta' => "Por gramaje o kilo",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Marqueta de chocolate semiamargo",
            'descripcion' => "Marqueta de chocolate semiamargo y frutos secos",
            'precio' => "$270",
            'imagen' => "/images/productos/marqueta-semiamarga.jpg",
            'imagen2' => "/images/productos/marqueta-2.jpg",
            'venta' => "Por gramaje o kilo",
            'activo' => "1"
        )
    ),
    'Confitados' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Almendras con chocolate blanco",
            'descripcion' => "Almendras cubiertas con chocolate blanco 32% cacao, leche y azúcar de caña añadida. El chocolate blanco es elaborado únicamente con manteca de cacao orgánica, Un snack delicioso y saludable, elaborado con cacao puro y orgánico de la más alta calidad.",
            'precio' => "$775 kg",
            'imagen' => "/images/productos/almendras-cacao-bco.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Almendras con chocolate semi amargo",
            'descripcion' => "Almendras tostadas cubiertas con chocolate semi amargo 32% cacao, con leche y azúcar de caña añadida. Un snack delicioso y saludable, libre de saborizantes y grasas añadidas. Elaborado con cacao puro y orgánico de la más alta calidad.",
            'precio' => "$775 kg",
            'imagen' => "/images/productos/almendras.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Almendras con chocolate de leche",
            'descripcion' => "Almendras tostadas cubiertas con chocolate semi amargo 70% cacao, azúcar de caña añadida y cacao en polvo natural. Un snack delicioso y saludable, libre de saborizantes y grasas añadidas. Elaborado con cacao puro y orgánico de la más alta calidad.",
            'precio' => "$775 kg",
            'imagen' => "/images/productos/almendras-choco.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        3 => array(
            'id' => 4,
            'nombre' => "Café con chocolate de leche",
            'descripcion' => "Café tostado cubierto con chocolate 38% cacao, leche y azúcar de caña añadida. Disfruta de la cremosidad del chocolate real y el intenso sabor del café, Un snack delicioso y saludable de la más alta calidad.",
            'precio' => "$700 kg",
            'imagen' => "/images/productos/cafe-confitado.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        4 => array(
            'id' => 5,
            'nombre' => "Dátil confitado",
            'descripcion' => "Un buen snack, almendras tostadas cubiertas con chocolate semi amargo.",
            'precio' => "$25",
            'imagen' => "/images/productos/datil.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "0"
        ),
        5 => array(
            'id' => 6,
            'nombre' => "Galleta relleno semiamargo y cubiertas con chocolate",
            'descripcion' => "Galleta relleno semiamargo y cubiertas con chocolate 100% cacao, 50gr.",
            'precio' => "$50",
            'imagen' => "/images/productos/galleta-rellena.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "0"
        ),
        6 => array(
            'id' => 7,
            'nombre' => "Nibs de cacao con chocolate blanco",
            'descripcion' => "Pedacitos crujientes al 32% de cacao cubiertos con chocolate blanco, con un sabor amargo ligero, listos para comerse.",
            'precio' => "$675 kg",
            'imagen' => "/images/productos/nibs-choco-bco.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        7 => array(
            'id' => 8,
            'nombre' => "Pasitas con chocolate semiamargo",
            'descripcion' => "Snack de pasas cubiertas con chocolate 38% cacao orgánico.",
            'precio' => "$675 kg",
            'imagen' => "/images/productos/pasitas.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        8 => array(
            'id' => 9,
            'nombre' => "Pasitas de chocolate de leche",
            'descripcion' => "Pasas cubiertas con chocolate 70% cacao orgánico con leche y azúcar de caña añadida, producto 100% mexicano de gran calidad.",
            'precio' => "$675 kg",
            'imagen' => "/images/productos/almendras-choco.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        )
    ),
    'Gotas' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Gotas de chocolate 100%",
            'descripcion' => "Gotas de chocolate 100% cacao",
            'precio' => "$570 kg",
            'imagen' => "/images/productos/gota-100.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        
        1 => array(
            'id' => 2,
            'nombre' => "Gotas de chocolate al 70%",
            'descripcion' => "Gotas de chocolate 70% - 100 gramos",
            'precio' => "$540 kg",
            'imagen' => "/images/productos/gota-70.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Gotas de chocolate al 38%",
            'descripcion' => "Gotas de chocolate 38% - 100 gramos",
            'precio' => "$540 kg",
            'imagen' => "/images/productos/gotas-choco-38.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        3 => array(
            'id' => 4,
            'nombre' => "Gotas de chocolate de maltitol",
            'descripcion' => "",
            'precio' => "$680 kg",
            'imagen' => "/images/productos/gota-maltitol.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        4 => array(
            'id' => 5,
            'nombre' => "Gotas de chocolate blanco",
            'descripcion' => "Gotas de chocolate blanco",
            'precio' => "$540 kg",
            'imagen' => "/images/productos/blanco.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        )
        
    ),
    'Granos' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Granos de chocolate semiamargo al 70% cacao",
            'descripcion' => "Grano Chocolate Semiamargo al 70% cacao  100 gramos",
            'precio' => "$110 kg",
            'imagen' => "/images/productos/grano-70.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Granos de chocolate blanco",
            'descripcion' => "Grano Chocolate blanco  100 gramos",
            'precio' => "$110 kg",
            'imagen' => "/images/productos/grano-bco.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Granos de chocolate de leche",
            'descripcion' => "Grano Chocolate de leche  100 gramos",
            'precio' => "$110 kg",
            'imagen' => "/images/productos/grano-leche.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        )
    ),
    'Manteca' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Manteca de cacao 1 kilo",
            'descripcion' => "Manteca de cacao 1 Kilo",
            'precio' => "$90",
            'imagen' => "/images/productos/manteca-cacao.jpg",
            'imagen2' => "",
            'venta' => "Gramaje",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Manteca de cacao orgánica",
            'descripcion' => "Manteca de cacao orgánica - 200 gramos",
            'precio' => "$160",
            'imagen' => "/images/productos/manteca-org.jpg",
            'imagen2' => "",
            'venta' => "Gramaje",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Manteca de cacao presentación 100 gramos",
            'descripcion' => "",
            'precio' => "$95",
            'imagen' => "/images/productos/manteca-envase.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        )
    ),
    'Frappes y Chocolate caliente' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Frappe de chocolate",
            'descripcion' => "Rico frappe de chocolate con nibs de cacao de topping",
            'precio' => "$90",
            'imagen' => "/images/productos/frappe.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Frappe de moka",
            'descripcion' => "Rico frappe de moka con nibs de cacao de topping",
            'precio' => "$90",
            'imagen' => "/images/productos/frappe.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Chocolate Caliente",
            'descripcion' => "Rico chocolate caliente",
            'precio' => "$100",
            'imagen' => "/images/productos/chocolate-caliente.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        )
    ),
    'Cacao, Nibs De Cacao Y Pasta De Cacao' => array(
        0 => array(
            'id' => 1, 
            'nombre' => "Semilla Blend de cacao",
            'descripcion' => "Grano de cacao blend, fermentado y secado.",
            'precio' => "$250 kg",
            'imagen' => "/images/productos/cacao.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Nibs de cacao ",
            'descripcion' => "Pedacitos crujientes de cacao tostado, con un sabor amargo ligero, listos para comerse.",
            'precio' => "$350 kg",
            'imagen' => "/images/productos/nibs-cacao.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Pasta de cacao",
            'descripcion' => "Cacao orgánico molido. A partir de la pasta de cacao surgen los chocolates. La pasta es simplemente el cacao molido, este se vuelve líquido por el calor y la fricción, posteriormente se deja reposar y vuelve a endurecerse. Cada chocolatero añade leche o endulzante, refina y convierte en un sedoso chocolate. Esta pasta de cacao, de la mejor calidad, podrás usarla para hacer tus propias creaciones chocolateras. Además, es un producto 100% mexicano.",
            'precio' => "$300 kg",
            'imagen' => "/images/productos/troseada.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        )
    ),
    'Polvos' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Cacao en polvo ",
            'descripcion' => "100% cacao molido.",
            'precio' => "$430 kg",
            'imagen' => "/images/productos/cacao-polvo.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Chocolate en polvo",
            'descripcion' => "Cacao en polvo, leche y azucar.",
            'precio' => "$280 kg",
            'imagen' => "/images/productos/cacao-polvo.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Cocoa alcalina",
            'descripcion' => "Cacao alcalinizado.",
            'precio' => "$510 kg",  
            'imagen' => "/images/productos/cacao-polvo.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        3 => array(
            'id' => 4,
            'nombre' => "Moka",
            'descripcion' => "Rica mezcla de cacao y café en polvo.",
            'precio' => "$350 kg",
            'imagen' => "/images/productos/cacao-polvo.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        )
    ),
    'Varios' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Bombas de chocolate",
            'descripcion' => "Bombas de Chocolate 55 gr",
            'precio' => "$100",
            'imagen' => "/images/productos/bomba.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "0"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Botellon de barro",
            'descripcion' => "",
            // Botellon de barro
            'precio' => "$980",
            'imagen' => "/images/productos/botellon.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Caballito tequilero",
            'descripcion' => "",
            // Caballito tequilero
            'precio' => "$75",
            'imagen' => "/images/productos/tequilero.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        3 => array(
            'id' => 4,
            'nombre' => "Chocolatera",
            'descripcion' => "",
            'precio' => "$500",
            'imagen' => "/images/productos/chocolatera.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "0"
        ),
        4 => array(
            'id' => 5,
            'nombre' => "Crema de moka o chocolate",
            'descripcion' => "",
            // Crema de moka o chocolate
            'precio' => "$375",
            'imagen' => "/images/productos/crema-moka.jpg",
            'imagen2' => "/images/productos/crema-moka-2.jpg",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        5 => array(
            'id' => 6,
            'nombre' => "Enjambre de amaranto semiamargo",
            'descripcion' => "Enjambre de Amaranto con Nueces y Chocolate Semiamargo 30 gr",
            'precio' => "$40",
            'imagen' => "/images/productos/enjambre.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "0"
        ),
        6 => array(
            'id' => 7,
            'nombre' => "Juego jarra con vasos",
            'descripcion' => "",
            'precio' => "$1320",
            'imagen' => "/images/productos/jarra-con-vasos.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "0"
        ),
        7 => array(
            'id' => 8,
            'nombre' => "Libro",
            'descripcion' => "LIBROS CHOCOLATES",
            'precio' => "$650",
            'imagen' => "/images/productos/libro.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        8 => array(
            'id' => 9,
            'nombre' => "Miel de abeja multifloral",
            'descripcion' => "Disfruta de una miel pura de abeja, multifloral 100% natural.",
            'precio' => "$175",
            'imagen' => "/images/productos/miel.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        9 => array(
            'id' => 10,
            'nombre' => "Miel 1.380 litros",
            'descripcion' => "",
            // Olla grande de barro
            'precio' => "$230",
            'imagen' => "/images/productos/miel-1litro.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        10 => array(
            'id' => 11,
            'nombre' => "Molinillo chico",
            'descripcion' => "Batidor especial para la preparación del chocolate caliente, siendo su principal función disolver el chocolate y producir espuma.",
            'precio' => "$65",
            'imagen' => "/images/productos/molino.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        11 => array(
            'id' => 12,
            'nombre' => "Molinillo mini",
            'descripcion' => "",
            // Molinillo mini
            'precio' => "$45",
            'imagen' => "/images/productos/molinillo-mini.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        12 => array(
            'id' => 13,
            'nombre' => "Molinillo mediano",
            'descripcion' => "Batidor especial para la preparación del chocolate caliente, siendo su principal función disolver el chocolate y producir espuma.",
            'precio' => "$130",
            'imagen' => "/images/productos/molino.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        13 => array(
            'id' => 14,
            'nombre' => "Molinillo grande ",
            'descripcion' => "",
            // Molinillo grande
            'precio' => "$160",
            'imagen' => "/images/productos/molinillo.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        
        14 => array(
            'id' => 15,
            'nombre' => "Olla grande con tapa de barro",
            'descripcion' => "",
            // Olla grande con tapa de barro
            'precio' => "$700",
            'imagen' => "/images/productos/olla-gde-tapa.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        15 => array(
            'id' => 16,
            'nombre' => "Conejitos de pascua",
            'descripcion' => "",
            'precio' => "$50",
            'imagen' => "/images/productos/conejito.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "0"
        ),
        16 => array(
            'id' => 17,
            'nombre' => "Separador de libros",
            'descripcion' => "",
            // Separador de libros
            'precio' => "$50",
            'imagen' => "/images/productos/separadores.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        17 => array(
            'id' => 18,
            'nombre' => "Vino tinto Calle Once",
            'descripcion' => "",
            'precio' => "$700",
            'imagen' => "/images/productos/vino-tinto.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        18 => array(
            'id' => 19,
            'nombre' => "Taza de barro colores",
            'descripcion' => "",
            'precio' => "$80",
            'imagen' => "/images/productos/taza-colores.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        19 => array(
            'id' => 20,
            'nombre' => "Velas de mazorca de cacao",
            'descripcion' => "",
            // Vela hecha en mazorca de cacao
            'precio' => "$250",
            'imagen' => "/images/productos/velas.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        20 => array(
            'id' => 21,
            'nombre' => "Mazorca de cerámica",
            'descripcion' => "",
            // Mazorca de ceramica
            'precio' => "$450",
            'imagen' => "/images/productos/mazorca-ceramica.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "0"
        ),
        20 => array(
            'id' => 21,
            'nombre' => "Tortuga de nuez y dátil 35gr.",
            'descripcion' => "Un buen snack, almendras tostadas cubiertas con chocolate semi amargo.",
            'precio' => "$50",
            'imagen' => "/images/productos/tortuga.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        21 => array(
            'id' => 22,
            'nombre' => "Taza cerámica con logo",
            'descripcion' => "",
            // Caja con 35 bombones de chocolate
            'precio' => "$170",
            'imagen' => "/images/productos/taza-ceramica.jpg",
            'imagen2' => "",
            'venta' => "Pieza",
            'activo' => "1"
        ),
        22 => array(
            'id' => 23,
            'nombre' => "Caja con 25 chocolates",
            'descripcion' => "Caja con 25 bombones de chocolate",
            'precio' => "Precio variable, pregunte en nuestra tienda.",
            'imagen' => "/images/productos/caja-25-1.jpg",
            'imagen2' => "/images/productos/caja-25-2.jpg",
            'venta' => "Caja",
            'activo' => "0"
        ),
        23 => array(
            'id' => 24,
            'nombre' => "Caja con 35 chocolates",
            'descripcion' => "",
            // Caja con 35 bombones de chocolate
            'precio' => "Precio variable, pregunte en nuestra tienda.",
            'imagen' => "/images/productos/caja-35.jpg",
            'imagen2' => "",
            'venta' => "Caja",
            'activo' => "0"
        ),
        24 => array(
            'id' => 25,
            'nombre' => "Caja con 50 chocolates",
            'descripcion' => "",
            // Caja con 35 bombones de chocolate
            'precio' => "Precio variable, pregunte en nuestra tienda.",
            'imagen' => "/images/productos/caja-50.jpg",
            'imagen2' => "/images/productos/caja-50-2.jpg",
            'venta' => "Caja",
            'activo' => "0"
        ),
        25 => array(
            'id' => 26,
            'nombre' => "Bolsitas bombones de colores rellenos",
            'descripcion' => "Bolsita Bombones diferentes colores 150 gr",
            'precio' => "$270",
            'imagen' => "/images/productos/bolsita-colores.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "0"
        ),
        26 => array(
            'id' => 27,
            'nombre' => "Taza Natural barro",
            'descripcion' => "",
            'precio' => "$90",
            'imagen' => "/images/productos/vaso-natural.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        27 => array(
            'id' => 28,
            'nombre' => "Experiencia Maridaje de chocolate obscuro con vino tinto",
            'descripcion' => "",
            // Experiencia Maridaje de chocolate obscuro con vino tinto
            'precio' => "500",
            'imagen' => "/images/productos/exp-vino.jpg",
            'imagen2' => "",
            'venta' => "Por persona",
            'activo' => "0"
        ),
        28 => array(
            'id' => 29,
            'nombre' => "Galleta rellena de chocolate semiamargo",
            'descripcion' => "Galleta rellena de chocolate semiamargo y cubiertas con chocolate 100% cacao, 50gr.",
            'precio' => "$50",
            'imagen' => "/images/productos/galleta-rellena.jpg",
            'imagen2' => "",
            'venta' => "Por gramaje",
            'activo' => "0"
        ),
        29 => array(
            'id' => 30,
            'nombre' => "Tejas de cacahuate chocolate amargo al 100% cacao",
            'descripcion' => "Un buen snack, almendras tostadas cubiertas con chocolate semi amargo 12gr.",
            'precio' => "$20",
            'imagen' => "/images/productos/tejas.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "0"
        ),
        31 => array(
            'id' => 32,
            'nombre' => "Mini plato con bombón",
            'descripcion' => "",
            // Mini Platito Con Bombón
            'precio' => "$50",
            'imagen' => "/images/productos/mini-plato.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        32 => array(
            'id' => 33,
            'nombre' => "Olla grande de barro",
            'descripcion' => "",
            // Olla grande de barro
            'precio' => "$600",
            'imagen' => "/images/productos/olla-gde-barro.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        33 => array(
            'id' => 34,
            'nombre' => "Paquete - Vino tinto con crema de moka y chocolate",
            'descripcion' => "",
            'precio' => "$1950",
            'imagen' => "/images/productos/vino-crema.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        

    ),
    'Especiales' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Conejitos de pascua",
            'descripcion' => "",
            // Conejitos de pascua
            'precio' => "$50",
            'imagen' => "/images/productos/conejito.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Dátil relleno de almendra",
            'descripcion' => "Dátil relleno de almendra y cubierto con chocolate semiamargo",
            // Caja con 50 bombones de chocolate
            'precio' => "$25",
            'imagen' => "/images/productos/datil.jpg",
            'imagen2' => "",
            'venta' => "Pieza",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Enjambre de amaranto semiamargo",
            'descripcion' => "Enjambre de Amaranto con Nueces y Chocolate Semiamargo 30 gr",
            'precio' => "$40",
            'imagen' => "/images/productos/enjambre.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),
        3 => array(
            'id' => 4,
            'nombre' => "Galleta rellena y cubiertas con chocolate semiamargo",
            'descripcion' => "Galleta relleno semiamargo y cubiertas con chocolate 100% cacao, 50gr.",
            'precio' => "$50",
            'imagen' => "/images/productos/galleta-rellena.jpg",
            'imagen2' => "",
            'venta' => "Por kilo o gramaje",
            'activo' => "1"
        ),
        4 => array(
            'id' => 5,
            'nombre' => "Bombas de chocolate",
            'descripcion' => "Bombas de Chocolate 55 gr",
            'precio' => "$100",
            'imagen' => "/images/productos/bomba.jpg",
            'imagen2' => "",
            'venta' => "Granel",
            'activo' => "1"
        ),


    ),
    'Empaque' => array(
        0 => array(
            'id' => 1,
            'nombre' => "Brocheta de bombones",
            'descripcion' => "",
            'precio' => "$35",
            'imagen' => "/images/productos/brocheta.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        1 => array(
            'id' => 2,
            'nombre' => "Canasta de colores",
            'descripcion' => "Empaque Canasta De Colores",
            'precio' => "$70",
            'imagen' => "/images/productos/canasta-colores.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        2 => array(
            'id' => 3,
            'nombre' => "Empaque canasta de bambú",
            'descripcion' => "",
            // Empaque canasta de bambú
            'precio' => "$35",
            'imagen' => "/images/productos/canasta-natural.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        3 => array(
            'id' => 4,
            'nombre' => "Cayuco grande en acuarela",
            'descripcion' => "",
            'precio' => "$500",
            'imagen' => "/images/productos/acuarela-gde.jpg",
            'imagen2' => "/images/productos/acuarela-2.jpg",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        4 => array(
            'id' => 5,
            'nombre' => "Cayuco mediano en acuarela",
            'descripcion' => "",
            'precio' => "$220",
            'imagen' => "/images/productos/cayuco-ac-chico.jpg",
            'imagen2' => "/images/productos/acuarela-2.jpg",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        5 => array(
            'id' => 6,
            'nombre' => "Cayuco grande pirograbado",
            'descripcion' => "",
            'precio' => "$350",
            'imagen' => "/images/productos/cayuco-gde.jpg?=1",
            'imagen2' => "/images/productos/pirograbado-2.jpg",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        6 => array(
            'id' => 7,
            'nombre' => "Cayuco mediano pirograbado",
            'descripcion' => "",
            'precio' => "$140",
            'imagen' => "/images/productos/cayuco-chico.jpg",
            'imagen2' => "/images/productos/pirograbado-2.jpg",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        7 => array(
            'id' => 8,
            'nombre' => "Cofre en acuarela",
            'descripcion' => "Empaque Cofre De Madera En Acuarela",
            'precio' => "$220",
            'imagen' => "/images/productos/cofre-acuarela.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        8 => array(
            'id' => 9,
            'nombre' => "Cofre pirograbado",
            'descripcion' => "Empaque Cofre De Madera Pirograbado",
            'precio' => "$250",
            'imagen' => "/images/productos/cofre-pirograbado.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        9 => array(
            'id' => 10,
            'nombre' => "Empaque mini canastilla",
            'descripcion' => "",
            'precio' => "$45",
            'imagen' => "/images/productos/empaque-mini.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "0"
        ),
        10 => array(
            'id' => 11,
            'nombre' => "Caja con 9 chocolates",
            'descripcion' => "",
            // Caja con 35 bombones de chocolate
            'precio' => "$162",
            'imagen' => "/images/productos/caja-9-col.jpg",
            'imagen2' => "",
            'venta' => "Caja",
            'activo' => "1"
        ),
        11 => array(
            'id' => 12,
            'nombre' => "Caja con 25 chocolates",
            'descripcion' => "Caja con 25 bombones de chocolate",
            'precio' => "$450",
            'imagen' => "/images/productos/caja-25-1.jpg",
            'imagen2' => "/images/productos/caja-25-2.jpg",
            'venta' => "Caja",
            'activo' => "1"
        ),
        12 => array(
            'id' => 13,
            'nombre' => "Caja con 35 chocolates",
            'descripcion' => "",
            // Caja con 35 bombones de chocolate
            'precio' => "$630",
            'imagen' => "/images/productos/caja-35.jpg",
            'imagen2' => "",
            'venta' => "Caja",
            'activo' => "1"
        ),
        13 => array(
            'id' => 14,
            'nombre' => "Caja con 50 chocolates",
            'descripcion' => "",
            // Caja con 35 bombones de chocolate
            'precio' => "$900",
            'imagen' => "/images/productos/caja-50.jpg",
            'imagen2' => "/images/productos/caja-50-2.jpg",
            'venta' => "Caja",
            'activo' => "1"
        ),
        14 => array(
            'id' => 15,
            'nombre' => "Caja con 9 chocolates Veganos",
            'descripcion' => "",
            // Caja con 35 bombones de chocolate
            'precio' => "$162",
            'imagen' => "/images/productos/caja-9.jpg",
            'imagen2' => "",
            'venta' => "Caja",
            'activo' => "1"
        ),
        15 => array(
            'id' => 16,
            'nombre' => "Bolsitas bombones de colores rellenos",
            'descripcion' => "Bolsita Bombones diferentes colores 150 gr",
            'precio' => "$270",
            'imagen' => "/images/productos/bolsita-colores.jpg",
            'imagen2' => "",
            'venta' => "Por pieza",
            'activo' => "1"
        ),
        
    ),

);